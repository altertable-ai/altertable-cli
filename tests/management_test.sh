#!/usr/bin/env bash
# Offline tests for the management API helpers (profile show / catalogs / api share these).
set -o pipefail
...
"${CLI}" profile --configure --api-key atm_stored --env production >/dev/null 2>&1
setup_http_log
setup_mock_http "${WHOAMI_MOCK}"
"${CLI}" api GET /whoami >/dev/null 2>&1
...
setup_http_log
setup_mock_http "${WHOAMI_MOCK}"
ALTERTABLE_API_KEY=atm_env "${CLI}" api GET /whoami >/dev/null 2>&1
...
echo "${OUT}" | grep -Fq 'Profile:' || fail "profile show: expected Profile line, got '${OUT}'"
echo "${OUT}" | grep -Fq 'No credentials configured' || fail "profile show: expected unset credentials hint, got '${OUT}'"
pass "profile show works without management API key"
