...
"${CLI}" profile create acme_staging --api-key atm_staging --env staging >/dev/null 2>&1
"${CLI}" profile create acme_production --api-key atm_prod --env production >/dev/null 2>&1
pass "profile create configures credentials for explicit profiles"
...
pass "profile switch acme_staging switches profile show identity"

setup_mock_http "${WHOAMI_PROD}"
OUT="$("${CLI}" --profile acme_production profile show 2>/dev/null)"
teardown_mock_http
...
OUT="$("${CLI}" profile create globex_dev --api-key atm_globex --env dev 2>/dev/null)"
echo "${OUT}" | grep -Fq 'configured' || fail "profile create should show configured status"
pass "profile create configures credentials"

# ── profile status verifies configured credentials and shows the profile ──
setup_mock_http '[{"urlPattern":"/whoami","method":"GET","body":"{\"principal\":{\"type\":\"User\",\"name\":\"Globex\",\"email\":\"g@x.io\"},\"organization\":{\"name\":\"Globex\",\"slug\":\"globex\"}}"}]'
OUT="$("${CLI}" profile status --name globex_dev 2>/dev/null)"
teardown_mock_http
echo "${OUT}" | grep -Fq 'Verification:' || fail "profile status should show verification: '${OUT}'"
pass "profile status verifies configured credentials"
