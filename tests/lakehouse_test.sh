...
setup_http_log
setup_mock_http "[
  {\"urlPattern\":\"/append\",\"method\":\"POST\",\"body\":\"{\\\"ok\\\":true,\\\"task_id\\\":\\\"${TASK_ID}\\\"}\"}
]"
...
echo "${OUT}" | jq -e '.ok == true and .task_id == "'"${TASK_ID}"'"' >/dev/null \
  || fail "append default: expected ok response with task id, got '${OUT}'"
teardown_http_log
pass "append defaults to the run command"

# ── append task subcommand ──
setup_http_log
setup_mock_http "[
  {\"urlPattern\":\"/tasks/${TASK_ID}\",\"method\":\"GET\",\"body\":\"{\\\"task_id\\\":\\\"${TASK_ID}\\\",\\\"status\\\":\\\"completed\\\"}\"}
]"
OUT="$("${CLI}" append task "${TASK_ID}" 2>/dev/null)"
teardown_mock_http
grep -q '^METHOD=GET$' "${HTTP_LOG}" || fail "append task: expected GET"
grep -q "^URL=https://lakehouse.example.test/tasks/${TASK_ID}$" "${HTTP_LOG}" \
  || fail "append task: wrong URL: $(grep '^URL=' "${HTTP_LOG}")"
echo "${OUT}" | jq -e '.task_id == "'"${TASK_ID}"'" and .status == "completed"' >/dev/null \
  || fail "append task: expected completed task response, got '${OUT}'"
teardown_http_log
pass "append task fetches status without append-run flags"
