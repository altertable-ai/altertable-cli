#!/usr/bin/env bash
# Offline tests for `altertable profile --configure` and `profile show`. No network, no real keychain.
set -o pipefail
...
"${CLI}" profile --configure --user u_blabla --password s_llll >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --basic-token dG9rZW4= >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --api-key atm_prod --env production >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u1 --password p1 >/dev/null 2>&1
"${CLI}" profile --configure --api-key atm_x --env prod >/dev/null 2>&1
...
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --api-key atm_x --env prod >/dev/null 2>&1
"${CLI}" profile --configure --api-key atm_y --env staging >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
if "${CLI}" profile --configure --user u --password p --api-key k --env e >/dev/null 2>&1; then
  fail "should reject combining mechanisms"
fi
pass "configure rejects combining authentication mechanisms"
if "${CLI}" profile --configure --user u --password p --env prod >/dev/null 2>&1; then
  fail "--env without --api-key should error"
fi
pass "--env is rejected without --api-key"
if "${CLI}" profile --configure --api-key k >/dev/null 2>&1; then
  fail "--api-key without --env should error"
fi
pass "--api-key without --env is rejected"
if "${CLI}" profile --configure --user u >/dev/null 2>&1; then
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
printf 's_fromstdin' | "${CLI}" profile --configure --user alice --password-stdin >/dev/null 2>&1
...
printf 'atm_fromstdin' | "${CLI}" profile --configure --api-key-stdin --env prod >/dev/null 2>&1
...
"${CLI}" profile show >/dev/null 2>&1 || fail "--show: should exit 0"
pass "--show shows the stored mechanism, masks secrets, names the secret store, exits 0"

# ── non-TTY bare profile --configure rejects without flags ──
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
ERR="$("${CLI}" profile --configure 2>&1 >/dev/null)" && fail "bare configure without TTY should error"
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user alice --password secret >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u --password p >/dev/null 2>&1
chmod 644 "${CRED_FILE}"
ERR="$("${CLI}" profile show 2>&1 >/dev/null)" && fail "--show should refuse a 644 credentials file"
...
chmod 600 "${CRED_FILE}"
"${CLI}" profile show >/dev/null 2>&1 || fail "--show should accept a 600 credentials file"
pass "a 600 credentials file is accepted"

# ── logout (non-interactive full reset) ──
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u --password p >/dev/null 2>&1
"${CLI}" logout >/dev/null 2>&1 || fail "--clear should exit 0"
...
echo "$("${CLI}" profile show 2>/dev/null)" | grep -q 'No credentials configured' || fail "--clear should leave no credentials"
pass "--clear removes all stored configuration without prompting"

# ── endpoints stored alongside a credential ──
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --api-key atm_x --env prod --control-plane-url http://localhost:13000 >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u --password p --data-plane-url http://localhost:15000 >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
ERR="$("${CLI}" profile --configure --control-plane-url http://localhost:13000 2>&1 >/dev/null)"
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --data-plane-url http://localhost:15000 >/dev/null 2>&1
...
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u --password p --data-plane-url http://127.0.0.1:1111 >/dev/null 2>&1
setup_http_log
setup_mock_http "${QUERY_MOCK}"
...
# ── profile show displays both planes ──
rm -f "${CONFIG_FILE}" "${CRED_FILE}"
rm -rf "${TEST_HOME}/profiles"
"${CLI}" profile --configure --user u --password p --data-plane-url http://localhost:15000 --control-plane-url http://localhost:13000 >/dev/null 2>&1
