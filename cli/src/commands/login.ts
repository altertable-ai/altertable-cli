...
// Identify the just-authenticated caller against a resolved management base URL,
// using the freshly minted token directly. Deliberately profile-free: the current
// profile may still hold a *different* org's stored session, and resolving auth or
// endpoints from it would make whoami report the wrong org and derive the wrong
// profile name.
...
// Re-fetch whoami through the now-current profile's stored session (the normal
// profile-based auth path) to confirm that what we persisted resolves to the
// same identity we just authenticated as.
...
// Whether two whoami responses describe the same authenticated identity and
// context: principal, organization, and environment.
